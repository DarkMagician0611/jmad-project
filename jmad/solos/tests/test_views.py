...
class SoloBaseTestCase(TestCase):
	...
	def setUpClass(cls):
		...
		cls.bugle_call_rag = Track.objects.create(
			name='Bugle Call Rag', slug='bugle_call_rag',
			album=cls.no_funny_hats)
		cls.drum_solo = Solo.objects.create(instrument='drums',
			artist='Rich', track=cls.bugle_call_rag, slug='rich')
		...
		cls.sax_solo = Solo.objects.create(instrument='saxophone',
			artist='Coltrane', track=cls.mr_pc, slug='coltrane')
...
class SoloViewTestCase(SoloBaseTestCase):
	...
	def test_basic(self):
		"""
		Test that the solo view returns a 200 response, uses the correct template, and 
